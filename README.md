...
按 `.env.example` 配置模型服务；默认使用本机 Ollama，需自行准备对应模型。Local Embedding 是确定性的 Hash 基线，便于离线运行，不等价于语义向量模型。
...
`.env` 是每位使用者自己的运行配置，不随仓库提交；`.env.example` 只提供变量名和无密钥的默认值。项目默认采用确定性的 Local Hash Embedding 作为离线基线，但回答模型、模型版本、资料内容和索引状态都会影响最终回答。因此，本项目可复现的是运行流程、资料处理和验证方法，而不是承诺不同环境下逐字相同的模型回答。
