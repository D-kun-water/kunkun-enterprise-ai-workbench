...
按 `.env.example` 配置模型服务；默认使用本机 Ollama 的 `qwen3.5:9b` 生成回答、`nomic-embed-text` 生成语义向量，需自行准备对应模型。Local Hash Embedding 只用于离线回归或模型服务不可用时的回退，不等价于语义向量模型。
...
`.env` 是每位使用者自己的运行配置，不随仓库提交；`.env.example` 只提供变量名和无密钥的默认值。项目默认采用 Ollama 语义 Embedding；可在离线回归或模型服务不可用时改用确定性的 Local Hash 基线。回答模型、模型版本、资料内容和索引状态都会影响最终回答。因此，本项目可复现的是运行流程、资料处理和验证方法，而不是承诺不同环境下逐字相同的模型回答。
